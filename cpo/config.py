...
class CPOConfig(TrainingArguments):
    ...
    per_sample_loss_jsonl_path: Optional[str] = None
    per_sample_loss_log_steps: int = 0

    easy_per_sample_jsonl_path: Optional[str] = None
    easy_per_sample_log_steps: Optional[int] = None

    def __post_init__(self):
        if self.loss_type == "kto_pair":
            raise ValueError("Support for kto_pair has been removed in DPOTrainer. Please use KTOTrainer.")
        if self.per_sample_loss_jsonl_path is None and self.easy_per_sample_jsonl_path:
            self.per_sample_loss_jsonl_path = self.easy_per_sample_jsonl_path
        if self.easy_per_sample_log_steps is not None:
            self.per_sample_loss_log_steps = self.easy_per_sample_log_steps
