...
def _load_dataset_split(ds: str, ds_config: Optional[str], split: str):
    """
    Load a split from Hub or disk. On Hub rate limits (429) or similar, retry cache-only
    so multi-GPU jobs do not all hit the API at once.
    """
    local_split_path = os.path.join(ds, split)
    try:
        return load_dataset(ds, ds_config, split=split)
...
def mix_datasets(
    ...
    for (ds, frac), ds_config in zip(dataset_mixer.items(), configs):
        fracs.append(frac)
        for split in splits:
            dataset = _load_dataset_split(ds, ds_config, split)
